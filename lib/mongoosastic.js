...
module.exports = function elasticSearchPlugin(schema, options){
  ...
  */
  ...
  function setUpRiver(schema) {
    schema.statics.river = function(cb, opts) {
      var model = this;
      setIndexNameIfUnset(model.modelName);
      if (!this.db.name) throw "ERROR: "+ model.modelName +".river() call before mongoose.connect"
      var host = opts.host || this.db.host;
      var port = opts.port || this.db.port;
      var authDB = opts.auth.db || this.db.name;
      var authUser = opts.auth.user || this.db.user;
      var authPass = opts.auth.password || this.db.pass;
      esClient.putRiver(
        'mongodb',
        indexName,
        {
          ...
          mongodb: {
            ...
            credentials: [
              { db: authDB, user: authUser, password: authPass }
            ],
            db: this.db.name,
